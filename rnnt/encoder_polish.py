...
class BaseEncoder(nn.Module):
    ...
    def forward(self, inputs, input_lengths):
        ...
        outputs, hidden = self.lstm(inputs) # hidden = [batch size, n layers * num directions, hid dim]

        if input_lengths is not None:
            _, desorted_indices = torch.sort(indices, descending=False)
            outputs, _ = nn.utils.rnn.pad_packed_sequence(outputs, batch_first=True)
            outputs = outputs[desorted_indices] # outputs = [batch size, src len, 2 * hidden_size]

        logits = self.output_proj(outputs)  # logits = [batch size, src len, output size]
